...
namespace GameEngine
{
    ...
    public:
        ...
        void add_event_listener(std::function<void(EventType&)> callback)
        {
            ...
            _eventCallbacks[static_cast<size_t>(EventType::type)] = std::move(baseCallback);
        }

        void dispatch(BaseEvent& event)
        {
            auto& callback = _eventCallbacks[static_cast<size_t>(event.get_type())];
            if (callback)
            {
                callback(event);
            }
    ...
    private:
        std::array<std::function<void(BaseEvent&)>, static_cast<size_t>(EventType::EventsCount)> _eventCallbacks;
    };
